(function ($, TM) {
	...
	module('jQuery.tMgmt - usage', function(hook) {
		...
		test('if an interval w/callback (and no incrementor) is set, it should be able to be manually cleared, and callback triggered when interval is manually cancelled', function(assert){
			window.calltest = 0;
			console.log('window.calltest set to '+window.calltest);
			...
			setTimeout(function(){
				...
				assert.equal(window.calltest, 4, 'callback was forced');
				done();
			}, 1600);
		});
